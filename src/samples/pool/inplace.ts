...
class InPlace{
  ...
  static MAT4_SCALE(a :mat4, x: number, y: number, z: number) {
  ...
  static MAT4_ROTATE_X(a: mat4, t: number) {
    const c = Math.cos(t);
    const s = Math.sin(t);
  ...
  static MAT4_ROTATE_Y(a: mat4, t: number) {
    const c = Math.cos(t);
    const s = Math.sin(t);
  ...
  static MAT4_ROTATE_Z(a: mat4, t: number) {
    const c = Math.cos(t);
    const s = Math.sin(t);
  ...
  static MAT4_TRANSFORM(position: vec3, rotation: vec3, scale: vec3,matrix: mat4) {
    
    Utils.MAT4_TRANSLATE(position[0], position[1], position[2], matrix);
    InPlace.MAT4_ROTATE_Y(matrix, rotation[1]);
    InPlace.MAT4_ROTATE_X(matrix, rotation[0]);
    InPlace.MAT4_ROTATE_Z(matrix, rotation[2]);
    InPlace.MAT4_SCALE(matrix, scale[0], scale[1], scale[2]);
  }

  static QUAT_MULTIPLY_BY_VEC3(q:vec4, v:vec3, o = Utils.VEC3_CREATE(0,0,0)):vec3
  {
    // Extract the vector part of the quaternion
    const u = Utils.VEC3_CREATE(q[0], q[1], q[2]);

    // Extract the scalar part of the quaternion
    const s = q[3];

    // Do the math
    return Utils.VEC3_ADD(Utils.VEC3_ADD(Utils.VEC3_SCALE(u, 2 * Utils.VEC3_DOT(u, v)), Utils.VEC3_SCALE(v, (s*s - Utils.VEC3_DOT(u, u)))),Utils.VEC3_SCALE(Utils.VEC3_CROSS(u,v), 2.0*s), o);
  }

  
  static MAT4_MULTIPLY_BY_QUAT(a :mat4, q:vec4){
