...
*/	
	...
	"vColor = vec4( customColor, 0.5 );",
	//"vAngle = 0.0;",

	"vec4 mvPosition = modelViewMatrix * vec4( position, 1.0 );",
	"gl_PointSize = customSize * ( 100.0 / mvPosition.w );",     // scale particles as objects in 3D space
...
{	
	constructor(scene, positionBase)
	{
		/////////////////////////
		// PARTICLE PROPERTIES //
		/////////////////////////
		...
		this.particleGeometry.setAttribute('customVisible', new THREE.Float32BufferAttribute([],1));
		this.particleGeometry.setAttribute('customAngle', new THREE.Float32BufferAttribute([],1));
		this.particleGeometry.setAttribute('customSize', new THREE.Float32BufferAttribute([],1));
		this.particleGeometry.setAttribute('customColor',	new THREE.Float32BufferAttribute([],3));
		this.particleGeometry.setAttribute('customOpacity',	new THREE.Float32BufferAttribute([],1));
		

		this.particleTexture  = new THREE.TextureLoader().load( '/assets/textures/smokeparticle.png' );
	...
	setValues ( parameters )
	{
		...
		this.emitterAge      = 0.0;
		this.emitterAlive    = true;
		this.particleCount = this.particlesPerSecond * Math.min( this.particleDeathAge, this.emitterDeathAge ) * 10;
		

		this.particleGeometry = new THREE.BufferGeometry();
		this.particleGeometry.setAttribute('position',	new THREE.Float32BufferAttribute(this.particleCount * 3, 3));
		this.particleGeometry.setAttribute('customVisible', new THREE.Float32BufferAttribute(this.particleCount,1));
		this.particleGeometry.setAttribute('customAngle', new THREE.Float32BufferAttribute(this.particleCount,1));
		this.particleGeometry.setAttribute('customSize', new THREE.Float32BufferAttribute(this.particleCount,1));
		this.particleGeometry.setAttribute('customColor',	new THREE.Float32BufferAttribute(this.particleCount * 3,3));
		this.particleGeometry.setAttribute('customOpacity',	new THREE.Float32BufferAttribute(this.particleCount,1));
	...
	initialize()
	{
		...
		for (var i = 0; i < this.particleCount; i++)
		{
			...
			this.particleGeometry.attributes.position[i] = this.particleArray[i].position;
			...
			console.log('initialize particle');
			console.log(JSON.stringify(this.particleArray[i]));
		}
	...
	update(dt)
	{
		...
		for (var i = 0; i < this.particleCount; i++)
		{
			if ( this.particleArray[i].alive )
			{
				...
				positions[i*3+0]= this.particleArray[i].position.x;
				...
				this.particleGeometry.attributes.customVisible[i] = this.particleArray[i].alive;

				this.particleGeometry.attributes.customColor.array[i *3+0]   = this.particleArray[i].color.r;
				this.particleGeometry.attributes.customColor.array[i *3+1]   = this.particleArray[i].color.g;
				this.particleGeometry.attributes.customColor.array[i *3+2]   = this.particleArray[i].color.b;

				this.particleGeometry.attributes.customOpacity[i] = this.particleArray[i].opacity;
				this.particleGeometry.attributes.customSize[i]    = this.particleArray[i].size;
				this.particleGeometry.attributes.customAngle[i]   = this.particleArray[i].angle;
			}		
		}
		...
		for (var j = 0; j < recycleIndices.length; j++)
		{
			...
			/*
			this.particleGeometry.attributes.position[i * 3 + 0] = this.particleArray[i].position.x;
			this.particleGeometry.attributes.position[i * 3 + 1] = this.particleArray[i].position.y;
			this.particleGeometry.attributes.position[i * 3 + 2] = this.particleArray[i].position.z;
			*/
...
export {ParticleEngine,Particle, Tween, Type};
///////////////////////////////////////////////////////////////////////////////
