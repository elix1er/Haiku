...
class SampleState extends State {
  ...
  onEnter() {
    this.ecs = new ECSManager(this.app);
    this.ecs.addSystem(new MoveSystem(this.app));
    this.ecs.addSystem(new DisplaySystem(this.app));
  ...
  createBall(color, x, y, vx, vy) {
    let entity = new ECSEntity(this.app);
